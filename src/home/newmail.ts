...
export const isNewWindow: boolean = true;
export let selectedToolbarItem: string = '';

window.newmail = (): void => {
    renderToolbar();
    createMailForm();
    const mentionObject: Mention = new Mention({
...
function renderToolbar(): void {
    const moreTemplate: string = '<input type="text" tabindex="1" id="moreList2" />';
...
function renderMoreList(): void {
    const themeList: { [key: string]: unknown }[] = [
...
export function showMailDialog(option: string, selectedMessage: { [key: string]: unknown }): void {
    clearMailForm();
    let key: string = '';
    toolbarHeader1.refreshOverflow();
